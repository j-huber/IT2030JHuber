...
namespace EnrollmentApplication.Models
{
    public class Enrollment : IValidatableObject
    {
        ...
        [Required]
        [Range(2018, 9999, ErrorMessage ="The year entered cannot be before 2018.")]
        ...
        {
            var invalidCharacterList = "*&^%$#@!()[]{}";
            var NotesArray = Notes.ToCharArray();
            var CharListArray = invalidCharacterList.ToCharArray();
            for (int i = 0; i <= NotesArray.Length; i++)
            {
                for (int j = 0; j <= CharListArray.Length; j++)
                {
                    ...
                    if (equals == true)
                    {
                        yield return (new ValidationResult("You must not use special characters."));
                    }
                }
            }
